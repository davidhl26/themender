# PROMPTS — SÉQUENCE 10  ·  Seedance 2.0 · 1080p

> Gabarit validé en vrai sur 1B le 06/09. **C'est cette version qu'on colle.**
> Le détail complet reste dans `PRET-SEQ-10.md` (géométrie GEO, verrous, raccords).

> ⚠ **Toutes les mentions `@` sont sur les deux premières lignes.** Colle le texte, puis remonte
> et **re-sélectionne chacune dans le sélecteur** : un `@` collé en texte brut ne se lie à rien.

## 10A — plan 10.1 « La maison rouge, et la porte » *(8 s → 4 s · @Nora + @RedHouseExterior · start frames LIEU-15 IMAGE 1 puis IMAGE 2 · 40 mm f/2.8)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **8 s** · **son ON** |
| **`@image1`** (start image) | LIEU-15 IMAGE 1 puis IMAGE 2 |
| **`@video1`** | — aucune vidéo, tête de chaîne |
| **Éléments** | `@nora` · `@RedHouseExterior` |

```
MATERIALS — @image1 IS THE FIRST FRAME OF THIS GENERATION. No previous clip is attached: this shot is the head of its chain and sets the light, the grain and the skin rendering the following shots match. @nora is NORA. @RedHouseExterior is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From NORA and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — NORA walks the last metres of a grey street toward the only red house in the world, stops, lays her hand flat on the door — and the door opens under her hand.
OPENING FRAME — very wide straight down the street, eye level 160 cm, locked off, ref axis; the red house centred x=50%, 55% h at the far end.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-2.4s] START: NORA enters bottom of frame at x=48%, from directly behind, 22% h, walking away down the middle of the road; Six unhurried steps away; [2.4-4.8s] She stops before the house, back to camera, shoulders lifting once on a breath. She stands 3/4 back, looking at the door. [4.8-7.2s] Her right hand rises and lays flat on the board at chest height, fingers spread. The door gives and swings inward 40 cm on its own weight;
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 40 mm f/2.8 equivalent. CAMERA HOLD, and it is not optional: the camera stays very wide straight down the street, eye level 160 cm, locked off, ref axis; the red house centred x=50%, 55% h at the far end.
LIGHT — STATE A · THE WINDOW LIT, THE DOOR SHUT (the arrival).
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The door 40 cm open, warm light widening across her cheek and the porch boards, her hand still flat on the board, the street black behind her.
AVOID — a paraffin flame, a flickering flame, candlelight, knocking, a doorbell, a second person, any red outside the house, red on figure, the door opening before her hand touches it, readable words.
```

## 10B — plan 10.2 « Des écritures partout » *(12 s → 7 s · @Nora + @RedHouseInterior v2 · start frame IMAGE A v2 · 28 mm f/1.4)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **12 s** · **son ON** |
| **`@image1`** (start image) | IMAGE A v2 |
| **`@video1`** | le clip précédent |
| **Éléments** | `@nora` · `@redhouseinterior` |

```
MATERIALS — @video1 is the shot immediately before this one (10A). @image1 IS THE FIRST FRAME OF THIS GENERATION. It is NOT the last frame of the attached video: this shot is taken from a different angle and its opening composition is the one @image1 gives. @nora is NORA. @redhouseinterior is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the attached video take ONLY the film stock, the grain structure, the way skin and fabric resolve, the focus behaviour and the highlight roll-off. DO NOT TAKE its light, its palette, its exposure, its composition or its framing, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From NORA and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — NORA steps into a room written on for months and turns once on herself, far too slowly for a look. 12 seconds, one continuous framing, no cut.
CONTINUITY — ALIGN THE BOUNDARY BEFORE ANYTHING NEW HAPPENS: this clip opens on the INHERITED STATE line below exactly, everything in it already true and nothing replayed, and only then does the action begin. It must CONNECT NATURALLY, NOT IDENTICALLY.
INHERITED STATE, ALREADY TRUE AT FRAME ONE — NORA: left third of frame, STANDING, x=32%. Nothing in this list may be re-placed, re-lit or improved.
OPENING FRAME — wide from just inside the doorway, eye level 160 cm, locked off, ref axis; the bulb at the top of frame, the pots sharp along the lower frame edge.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-3.7s] START: the written room, the bulb burning, the pots, brushes, ladder and mattress all in frame. Her head goes up the left wall and further up, to the dark above the writing where the bulb light gives out. [3.7-7.3s] She turns once on herself — ONE single full revolution taking four and a half seconds, far too slow for a look; [7.3-11.0s] She comes to rest facing the far wall again, 3/4 back, both arms hanging.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 28 mm f/1.4 equivalent. CAMERA HOLD, and it is not optional: the camera stays wide from just inside the doorway, eye level 160 cm, locked off, ref axis; the bulb at the top of frame, the pots sharp along the lower frame edge.
LIGHT — TWO STATES, NOTHING IN BETWEEN.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — NORA 3/4 back to camera in the middle of the written room, arms hanging, the bulb burning above her, the pots and brushes sharp along the skirting, the ceiling black.
AVOID — paint pots absent, brushes absent, ladder absent, mattress absent, a clean room, a paraffin lamp, candles, writing above a man's height, writing on the ceiling, glowing walls.
```

## 10C — plans 10.3 + 10.4 « Le dragon, puis CLIC » *(10 s → 7 s · @Nora + @RedHouseInterior v2 · start frame shot 1 : IMAGE B v2 ; shot 2 : aucun · 28 mm f/1.4)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **10 s** · **son ON** |
| **`@image1`** (start image) | shot 1 : IMAGE B v2 ; shot 2 : aucun |
| **`@video1`** | le clip précédent |
| **Éléments** | `@nora` · `@redhouseinterior` |

```
MATERIALS — @video1 is the shot immediately before this one (10B). @image1 IS THE FIRST FRAME OF THIS GENERATION. It is NOT the last frame of the attached video: this shot is taken from a different angle and its opening composition is the one @image1 gives. @nora is NORA. @redhouseinterior is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the attached video take the light level and direction, the grain, the skin rendering and the way the camera behaves. TAKE NO FRAMING FROM IT beyond the first instant, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From NORA and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — NORA stops dead before one clear empty patch of plaster in the middle of the writing, smiles without knowing why, and walks on; at the door her hand finds the switch out of ordinary politeness and turns it off.
CONTINUITY — ALIGN THE BOUNDARY BEFORE ANYTHING NEW HAPPENS: this clip opens on the INHERITED STATE line below exactly, everything in it already true and nothing replayed, and only then does the action begin. It must CONNECT NATURALLY, NOT IDENTICALLY.
INHERITED STATE, ALREADY TRUE AT FRAME ONE — NORA: left third of frame, STANDING, x=30%. Nothing in this list may be re-placed, re-lit or improved.
OPENING FRAME — medium-close on the written wall, eye level, straight on, locked off, ref axis; the empty plaster patch centred x=50%, its top at 62% h.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-3.0s] START: the written wall and the empty patch, the bulb's cone raking the strokes. [3.0-6.0s] The corners of her mouth go up into a small unexplained smile, and she is already moving again — one step left, out of the near edge at [4.4s]. NORA enters from the left frame edge, from behind, 75% h, and stops at the door. [6.0-9.0s] Her right hand comes up and finds the switch without looking at it — the ordinary reflex of turning off other people's lights. Her fingers turn it.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 28 mm f/1.4 equivalent. CAMERA HOLD, and it is not optional: the camera stays medium-close on the written wall, eye level, straight on, locked off, ref axis; the empty plaster patch centred x=50%, its top at 62% h.
LIGHT — TWO STATES, NOTHING IN BETWEEN.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — Pure black — no image, no glow, no silhouette, nothing.
AVOID — a drawing on the plaster, a dragon, an animal, a creature, any figure on the wall, marks on the clear patch, figure touching the wall, a frown, recognition, the bulb lit after the click.
```

## 10D-1 — plan 10.5, segment 1/2 « Le noir s'allume » *(8 s · @Nora + @RedHouseInterior v2 · start frame : AUCUN, le bloc s'ouvre sur du noir plein · 28 mm f/1.4)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **8 s** · **son ON** |
| **`@image1`** (start image) | AUCUN, le bloc s'ouvre sur du noir plein |
| **`@video1`** | — aucune vidéo, tête de chaîne |
| **Éléments** | `@nora` · `@redhouseinterior` |

```
MATERIALS — @image1 IS THE FIRST FRAME OF THIS GENERATION. No previous clip is attached: this shot is the head of its chain and sets the light, the grain and the skin rendering the following shots match. @nora is NORA. @redhouseinterior is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From NORA and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — One full second of pure black, then the black lights up: the strokes she has just walked past rise slowly until the room is readable, and NORA in the middle of it stops breathing.
OPENING FRAME — wide, eye level 160 cm, middle of the room looking back toward the front door, locked off, the axis of reference IMAGE C. PURE BLACK. Nothing visible at all. Only her breathing, close.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-2.2s] The strokes begin to come up, very low, ember red under ash, first on the two side walls she walked past; [2.2-4.3s] The whole written band of both walls is up; [4.3-6.5s] The whole written band of both walls is up;
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 28 mm f/1.4 equivalent. CAMERA HOLD, and it is not optional: the camera stays wide, eye level 160 cm, middle of the room looking back toward the front door, locked off, the axis of reference IMAGE C.
LIGHT — TWO STATES, NOTHING IN BETWEEN.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The room at mid-glow, both written walls alive in dim ember red, the ceiling still dark, NORA a dark unmoving shape at x=50%, hand raised on the switch.
AVOID — letters appearing one by one, writing that draws itself, strokes crawling, a light source in frame, a lamp, a torch, moonlight, the bulb relighting, bright red, neon red.
```

## 10D-2 — plan 10.5, segment 2/2 « Le plafond entier » *(8 s · @Nora + @RedHouseInterior v2 · start frame : la LAST FRAME de 10D-1 · 28 mm f/1.4)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **8 s** · **son ON** |
| **`@image1`** (start image) | **la dernière frame du clip précédent** |
| **`@video1`** | le clip précédent |
| **Éléments** | `@nora` · `@redhouseinterior` |

```
MATERIALS — @video1 is the shot immediately before this one (10D-1). @image1 is the last frame of that shot and IT IS THE FIRST FRAME OF THIS GENERATION. @nora is NORA. @redhouseinterior is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the attached video take the light level and direction, the grain, the skin rendering and the way the camera behaves. TAKE NO FRAMING FROM IT beyond the first instant, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From NORA and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — The camera tilts up off the standing NORA and finds the ceiling: written corner to corner, brightest directly above the mattress, where a man lay for weeks with his arms over his face to finish it.
CONTINUITY — THE CAMERA IS ALREADY MOVING at the boundary. ALIGN THE BOUNDARY FIRST: start on that motion already underway, same speed, same line, no ease-in and no restart, then carry it on. One single move across both clips.
INHERITED STATE, ALREADY TRUE AT FRAME ONE — NORA: centre of frame, STANDING, x=50%. Nothing in this list may be re-placed, re-lit or improved.
OPENING FRAME — wide, eye level 160 cm, middle of the room looking back toward the front door, the exact frame that ends 10D-1; one single slow tilt up, no dolly, no zoom.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-2.2s] The start frame held exactly: both written walls glowing, NORA a dark shape at x=50%, 55% h, hand raised at x=62%. [2.2-4.4s] The camera tilts up 35 degrees over four seconds and a fifth, evenly, never accelerating: the tops of the walls pass, then the ceiling opens across the whole frame, written board by board corner to corner, brightest at x=54% directly above the mattress. [4.4-6.6s] The tilt stops on the ceiling;
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 28 mm f/1.4 equivalent. CAMERA MOVE, and it is not optional: Framing 1 [0.0-8.0s] — camera: wide, eye level 160 cm, middle of the room looking back toward the front door, the exact frame that ends 10D-1; one single slow tilt up, no dolly, no zoom.
LIGHT — TWO STATES, NOTHING IN BETWEEN.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The written ceiling filling the frame corner to corner in dim ember red, brightest above the mattress, the stair risers and the inside of the door holding the same red along the bottom.
AVOID — letters appearing one by one, writing that draws itself, strokes crawling, a light source in frame, a lamp, a torch, moonlight, the bulb relighting, bright red, neon red.
```

## 10E — plan 10.6 « Son visage » *(6 s → 5 s · @Nora + @RedHouseInterior v2 · start frame : AUCUN, écart déclaré · 50 mm f/1.4)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **6 s** · **son ON** |
| **`@image1`** (start image) | AUCUN, écart déclaré |
| **`@video1`** | le clip précédent |
| **Éléments** | `@nora` · `@redhouseinterior` |

```
MATERIALS — @video1 is the shot immediately before this one (10D-2). @image1 IS THE FIRST FRAME OF THIS GENERATION. It is NOT the last frame of the attached video: this shot is taken from a different angle and its opening composition is the one @image1 gives. @nora is NORA. @redhouseinterior is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the attached video take the light level and direction, the grain, the skin rendering and the way the camera behaves. TAKE NO FRAMING FROM IT beyond the first instant, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From NORA and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — The face of a who has just seen the inside of a whole house light up. 6 seconds, one continuous framing, no cut.
CONTINUITY — ALIGN THE BOUNDARY BEFORE ANYTHING NEW HAPPENS: this clip opens on the INHERITED STATE line below exactly, everything in it already true and nothing replayed, and only then does the action begin. It must CONNECT NATURALLY, NOT IDENTICALLY.
INHERITED STATE, ALREADY TRUE AT FRAME ONE — NORA: centre of frame, STANDING, x=50%. Nothing in this list may be re-placed, re-lit or improved.
OPENING FRAME — close, 90 cm from her, eye level, three-quarter, static, locked off; her face at x=48%, 78% h, the room behind her at razor-thin focus.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-1.8s] Her eyes go too fast — five or six saccades up and across, each 15 to 20 degrees, the head not moving at all. [1.8-3.6s] Her jaw drops open two centimetres and stays open. ONE tear leaves the outer corner of the right eye and runs the whole cheek to the jaw line. [3.6-5.4s] The corners of her mouth go up — a real smile arriving late and against the open jaw, the eyes creasing with it, one breath caught.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 50 mm f/1.4 equivalent. CAMERA HOLD, and it is not optional: the camera stays close, 90 cm from her, eye level, three-quarter, static, locked off; her face at x=48%, 78% h, the room behind her at razor-thin focus.
LIGHT — TWO STATES, NOTHING IN BETWEEN.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — Her face, close, in dim ember red — jaw open, one wet track down the right cheek, smiling, eyes creased, not moving.
AVOID — letters appearing one by one, writing that draws itself, strokes crawling, a light source in frame, a lamp, a torch, moonlight, the bulb relighting, bright red, neon red.
```

## 10F-1 — plan 10.7, segment 1/4 « La caméra la quitte » *(5 s · @Nora + @RedHouseInterior v2 · start frame : la LAST FRAME de 10E · 28 mm f/1.4)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **5 s** · **son ON** |
| **`@image1`** (start image) | **la dernière frame du clip précédent** |
| **`@video1`** | le clip précédent |
| **Éléments** | `@nora` · `@redhouseinterior` |

```
MATERIALS — @video1 is the shot immediately before this one (10E). @image1 is the last frame of that shot and IT IS THE FIRST FRAME OF THIS GENERATION. @nora is NORA. @redhouseinterior is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the attached video take the light level and direction, the grain, the skin rendering and the way the camera behaves. TAKE NO FRAMING FROM IT beyond the first instant, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From NORA and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — The camera leaves her face, draws back across the glowing room and out through the open door, and she stays where she is — smiling, getting smaller, until the door frame takes her.
CONTINUITY — THE CAMERA IS ALREADY MOVING at the boundary. ALIGN THE BOUNDARY FIRST: start on that motion already underway, same speed, same line, no ease-in and no restart, then carry it on. One single move across both clips.
OPENING FRAME — begins on the exact last frame of the previous generation, her face at x=48%, 78% h, then draws straight back at a constant 1.2 metres per second, eye level 160 cm, no pan, no tilt.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-1.5s] Her face recedes to 55% h; At [2.6s] she is 34% h at x=50%, the whole written room around her, the ceiling alight above, the mattress and the folded ladder crossing the lower frame. [1.5-2.9s] She holds at 24% h; [2.9-4.4s] The dark door jamb sweeps in from both edges and takes 30% of frame width on each side;
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 28 mm f/1.4 equivalent. CAMERA MOVE, and it is not optional: Framing 1 [0.0-5.0s] — camera: begins on the exact last frame of the previous generation, her face at x=48%, 78% h, then draws straight back at a constant 1.2 metres per second, eye level 160 cm, no pan, no tilt.
LIGHT — TWO STATES, NOTHING IN BETWEEN.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The open doorway seen from the porch boards, a dim red rectangle in a grey-black street, NORA a small unmoving shape inside it, the red stopping at the threshold.
AVOID — letters appearing one by one, writing that draws itself, strokes crawling, a light source in frame, a lamp, a torch, moonlight, the bulb relighting, bright red, neon red.
```

## 10F-2 — plan 10.7, segment 2/4 « La rue grise » *(5 s · @RedHouseExterior · start frame : la LAST FRAME de 10F-1 · 24 mm f/2.8)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **5 s** · **son ON** |
| **`@image1`** (start image) | **la dernière frame du clip précédent** |
| **`@video1`** | le clip précédent |
| **Éléments** | `@RedHouseExterior` |

```
MATERIALS — @video1 is the shot immediately before this one (10F-1). @image1 is the last frame of that shot and IT IS THE FIRST FRAME OF THIS GENERATION. @RedHouseExterior is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the attached video take the light level and direction, the grain, the skin rendering and the way the camera behaves. TAKE NO FRAMING FROM IT beyond the first instant, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From the references and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — The camera keeps going: back down the grey street NORA climbed, the red doorway shrinking to a warm point, and the ground begins to fall away. 5 seconds, one continuous framing, no cut.
CONTINUITY — THE CAMERA IS ALREADY MOVING at the boundary. ALIGN THE BOUNDARY FIRST: start on that motion already underway, same speed, same line, no ease-in and no restart, then carry it on. One single move across both clips.
OPENING FRAME — begins on the exact last frame of the previous generation, the open red doorway at x=50%, and draws straight back down the middle of the road at a constant 4 m/s while climbing at a constant 1.6 m/s; no pan, no zoom.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-1.3s] The doorway shrinks to 20% h; A sodium lamp passes overhead at [2.0s]; [1.3-2.7s] The camera is level with the first-floor windows; [2.7-4.0s] The camera is level with the first-floor windows;
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 24 mm f/2.8 equivalent. CAMERA MOVE, and it is not optional: Framing 1 [0.0-5.0s] — camera: begins on the exact last frame of the previous generation, the open red doorway at x=50%, and draws straight back down the middle of the road at a constant 4 m/s while climbing at a constant 1.6 m/s; no pan, no zoom.
LIGHT — STATE A · THE WINDOW LIT, THE DOOR SHUT (the arrival).
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — Roof slates and chimney stacks across the lower half of frame, the black channel of the street between them, the red doorway a dim point far below at x=50%.
AVOID — people in the street, figure, a man, a car moving, red on the road, red on the walls, the camera stopping or accelerating, drone propeller sound, readable words, legible handwriting.
```

## 10F-3 — plan 10.7, segment 3/4 « La verticale » *(5 s · aucun personnage · start frame : la LAST FRAME de 10F-2 · 24 mm f/2.8)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **5 s** · **son ON** |
| **`@image1`** (start image) | **la dernière frame du clip précédent** |
| **`@video1`** | le clip précédent |
| **Éléments** | — |

```
MATERIALS — @video1 is the shot immediately before this one (10F-2). @image1 is the last frame of that shot and IT IS THE FIRST FRAME OF THIS GENERATION.
ROLES — From the attached video take the light level and direction, the grain, the skin rendering and the way the camera behaves. TAKE NO FRAMING FROM IT beyond the first instant, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout.
SHOT — The camera leaves the street behind and rises straight up until the town is nothing but a scatter of orange grains in a black landscape. 5 seconds, one continuous framing, no cut.
CONTINUITY — THE CAMERA IS ALREADY MOVING at the boundary. ALIGN THE BOUNDARY FIRST: start on that motion already underway, same speed, same line, no ease-in and no restart, then carry it on. One single move across both clips.
OPENING FRAME — begins on the exact last frame of the previous generation and climbs straight up at a constant 9 m/s, tilting down 20 degrees evenly across the segment so the town opens beneath; no pan, no roll, no zoom.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-1.3s] Roofs, chimney stacks and the black channel of the street directly below, filling the frame. The roofs fall away; [1.3-2.7s] Very high: the town is a grid of orange grains in a black landscape, the horizon entering the top of frame. [2.7-4.0s] Very high: the town is a grid of orange grains in a black landscape, the horizon entering the top of frame.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 24 mm f/2.8 equivalent. CAMERA MOVE, and it is not optional: Framing 1 [0.0-5.0s] — camera: begins on the exact last frame of the previous generation and climbs straight up at a constant 9 m/s, tilting down 20 degrees evenly across the segment so the town opens beneath; no pan, no roll, no zoom.
LIGHT — Available light only: sodium lamps below, going from lights to grains as the camera rises, and a lighter black sky with no moon and no stars.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The town small and even and very far below: slate roofs, black streets, sodium grains, the sky a lighter black with no stars.
AVOID — red anywhere, a red roof, a red light, city lights bright enough to read, a lit sign, moon, stars, drone propeller sound, the camera rolling, the camera hovering.
```

## 10F-4 — plan 10.7, segment 4/4 « La redescente sur l'abribus » *(5 s · @SamSDF + @BusShelter · start frame : AUCUN ; cible visuelle : LIEU-14 IMAGE 3 · 24 mm f/2.8)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **5 s** · **son ON** |
| **`@image1`** (start image) | **la dernière frame du clip précédent** |
| **`@video1`** | le clip précédent |
| **Éléments** | `@BusShelter` |

> 🔴 **Élément(s) inexistant(s) sur le compte : @SamSDF.** Ils sont écrits en clair dans le prompt (le modèle les construira depuis la description), mais l'identité ne sera pas verrouillée d'un plan à l'autre tant qu'ils ne sont pas créés.

```
MATERIALS — @video1 is the shot immediately before this one (10F-3). @image1 is the last frame of that shot and IT IS THE FIRST FRAME OF THIS GENERATION. @BusShelter is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the attached video take the light level and direction, the grain, the skin rendering and the way the camera behaves. TAKE NO FRAMING FROM IT beyond the first instant, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From the references and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — Kilometres away from the red house, the camera comes down out of the air and settles on a bus shelter, where a man sits alone on the bench, head down. 5 seconds, one continuous framing, no cut.
CONTINUITY — THE CAMERA IS ALREADY MOVING at the boundary. ALIGN THE BOUNDARY FIRST: start on that motion already underway, same speed, same line, no ease-in and no restart, then carry it on. One single move across both clips.
OPENING FRAME — high above the town, descending on one straight line at a constant 11 m/s, tilting up 15 degrees evenly to level out, coming to rest at 12 m; no pan, no roll, no zoom.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-1.4s] The high town, orange grains on black, one grain isolating itself near frame centre. Descending: the grains resolve into lamps; [1.4-2.8s] Descending: the grains resolve into lamps; [2.8-4.2s] Down onto it: a bus shelter under one sodium lamp, its light box blank and lit, and a man seated on the bench, small, head down, unmoving — no facial feature readable.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 24 mm f/2.8 equivalent. CAMERA MOVE, and it is not optional: Framing 1 [0.0-5.0s] — camera: high above the town, descending on one straight line at a constant 11 m/s, tilting up 15 degrees evenly to level out, coming to rest at 12 m; no pan, no roll, no zoom.
LIGHT — one source only: a single sodium street lamp on a column above the shelter, the column out of the top of frame, mounted SLIGHTLY FORWARD OF THE BENCH LINE toward the road, that is, slightly on the camera's side, so the hard orange cone falls from above and a little toward the lens and never from behind — this is the whole point of the lighting: the brow does not shadow the eye sockets.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The shelter at rest in its sodium cone, the seated man a dark shape on the bench with his head down, the blank light box lit beside him, black road and black country all around.
AVOID — the man's face, his eyes, the man looking up, the man standing, a readable face at distance, red anywhere, tail lights, a traffic light, lettering on the light box, a bus.
```

## 10H — plan 10.8 « Les doigts, et les yeux » *(10 s → 6 s au montage · @SamSDF + @Sam + @BusShelter (LIEU-14) · Genre Drama · Camera Fine Film · 21:9 · 1080p · sound on · 85 mm f/2 · pas de start frame)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **10 s** · **son ON** |
| **`@image1`** (start image) | aucune — ce plan démarre sans image de départ |
| **`@video1`** | le clip précédent |
| **Éléments** | `@sam` · `@BusShelter` |

> 🔴 **Élément(s) inexistant(s) sur le compte : @SamSDF.** Ils sont écrits en clair dans le prompt (le modèle les construira depuis la description), mais l'identité ne sera pas verrouillée d'un plan à l'autre tant qu'ils ne sont pas créés.

```
MATERIALS — @video1 is the shot immediately before this one (10F-4). @image1 IS THE FIRST FRAME OF THIS GENERATION. It is NOT the last frame of the attached video: this shot is taken from a different angle and its opening composition is the one @image1 gives. @sam is SAM. @BusShelter is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the attached video take the light level and direction, the grain, the skin rendering and the way the camera behaves. TAKE NO FRAMING FROM IT beyond the first instant, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From SAM and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — A homeless man alone under sodium puts down a half-eaten sandwich, pushes his beanie back, wipes one cheek; his hands fill the frame half a second — the fingers stained dark to the second knuckle — and then the head lifts and his eyes come into the light.
CONTINUITY — ALIGN THE BOUNDARY BEFORE ANYTHING NEW HAPPENS: this clip opens on the first-frame image exactly, everything in it already true and nothing replayed, and only then does the action begin. It must CONNECT NATURALLY, NOT IDENTICALLY.
OPENING FRAME — Medium close, three-quarter from his left, camera 105 cm high, 1.6 m away, static, locked off. He fills 70% of frame height at x=44%, clouded glass behind, sodium raking from above frame.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-2.5s] Nothing moves but his breathing; The same hand PUSHES THE HOOD BACK, then the beanie back high on the crown: the forehead comes clear, the hair beneath soaked flat with sweat. [2.5-5.1s] His right hand WIPES ONE CHEEK with the rag, once, slowly, cheekbone down into the beard — the grime lifting in a band, the skin left lighter. [5.1-7.6s] Head down as before, hood back, beanie high on the crown, forehead and wet hair in the light, the eyes still lost in the down-tilt of the face.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 85 mm f/2 equivalent. CAMERA HOLD, and it is not optional: the camera stays Medium close, three-quarter from his left, camera 105 cm high, 1.6 m away, static, locked off.
LIGHT — one source only: a single sodium street lamp on a column above the shelter, the column out of the top of frame, mounted SLIGHTLY FORWARD OF THE BENCH LINE toward the road, that is, slightly on the camera's side, so the hard orange cone falls from above and a little toward the lens and never from behind — this is the whole point of the lighting: the brow does not shadow the eye sockets.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The bearded man alone on the bench under the sodium cone, head fully lifted, pale grey-blue eyes wide open in the orange light, looking up the black road past the lens; his hands open on the blankets, fingers stained brown-black to the second knuckle, the pale-grey packet against his left thigh.
AVOID — a false beard, a glued-on beard, a prosthetic beard, a beard being pulled off, a wig, a disguise being removed, any other person in frame, figure, a vehicle, headlights.
```

## 10I — montage 10.9, micro-plans **a + b + f + h** « SA PROPRE MAISON » *(14 s de rushes → 7,5 s au montage · @Sam + @Nora + @Mender + @NoraBedroom + @Kitchen + @Bathroom · 50 mm f/2 · start frame du shot 1 : **LA LAST FRAME DE LA GÉNÉRATION 4A** — LIEU-08 IMAGE 2, « LE CADRE DU PÈRE »)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **14 s** · **son ON** |
| **`@image1`** (start image) | du shot 1 : **LA LAST FRAME DE LA GÉNÉRATION 4A** — LIEU-08 IMAGE 2, « LE CADRE DU PÈRE » |
| **`@video1`** | le clip précédent |
| **Éléments** | `@sam` · `@nora` · `@mender` · `@NoraBedroom` · `@Kitchen-1` · `@Bathroom` |

```
MATERIALS — @video1 is the shot immediately before this one (10H). @image1 IS THE FIRST FRAME OF THIS GENERATION. It is NOT the last frame of the attached video: this shot is taken from a different angle and its opening composition is the one @image1 gives. @sam is SAM. @nora is NORA. @mender is THE MENDER. @NoraBedroom is NORA BEDROOM, one of the places this shot happens in. @Kitchen-1 is KITCHEN, one of the places this shot happens in. @Bathroom is BATHROOM, one of the places this shot happens in.
ROLES — From the attached video take ONLY the film stock, the grain structure, the way skin and fabric resolve, the focus behaviour and the highlight roll-off. DO NOT TAKE its light, its palette, its exposure, its composition or its framing, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From SAM, NORA, THE MENDER and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — Four moments in one house, cut hard together.
CONTINUITY — ALIGN THE BOUNDARY BEFORE ANYTHING NEW HAPPENS: this clip opens on the first-frame image exactly, everything in it already true and nothing replayed, and only then does the action begin. It must CONNECT NATURALLY, NOT IDENTICALLY.
OPENING FRAME — The locked frame of the reference start image: a near-black hallway at night, camera 160 cm high, 2.8 m from the door, axis 10 degrees off the wall.
ACTION in 4 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-3.0s] EXACTLY the reference frame, no movement of any kind: inside the warm band at x=39%, NORA curled on her side 2.5 m beyond the door, filling 24% of frame height, three-quarter from behind, her face half buried in the pillow and unreadable, her shoulders shaking softly; THE CAMERA PIVOTS SLOWLY RIGHT — one smooth pan, no dolly, no zoom, the gap sliding off frame-left — AND SAM IS THERE, one metre from the door, flat against the dark wall at x=58%, filling 74% of frame height, three-quarter to camera. [3.0-6.0s] HIS RIGHT HAND LIFTS toward the door — the back of the hand catching the light, THE FINGERS COMPLETELY CLEAN, no ink, no stain — AND STOPS twenty centimetres short of the wood. [6.0-9.0s] THE PEN RUNS: one unbroken stroke drawn slowly across the weave left to right, the ivory ink laying down wet and catching the lamp, the canvas giving under the nib; THE LOADED FINGERTIP DRAWS THREE SHORT HORIZONTAL BANDS across the dry glass, one under the other, slow and deliberate. [9.0-12.0s] THE COATED BACK IS ALREADY IN FRAME AND ALREADY MOVING, large — no entrance, no start-up: at x=52%, filling 76% of frame height, seen from DIRECTLY BEHIND, mid-stride, walking away from camera.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 50 mm f/2 equivalent. CAMERA HOLD, and it is not optional: the camera stays The locked frame of the reference start image: a near-black hallway at night, camera 160 cm high, 2.8 m from the door, axis 10 degrees off the wall.
LIGHT — Shot 1: one source, the warm sodium-orange band escaping the door gap; it dies within a metre and NOTHING ABOVE HIS CHEST IS EVER TOUCHED BY IT — his head stays in true unlit black, no fill, no bounce, no rim.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The empty near-black hallway seen from the bedroom end, the far corner at x=24% with one thin thread of sodium orange on the wall, the unseen stairs dropping beyond it — nobody in frame, nothing moving.
AVOID — the father's face, his face lit, light on his eyes, his eyes, his eyebrows, any part of his face above the upper lip, a second light source in the hallway, figure turning toward the door, figure sitting up, figure seeing anyone.
```

## 10J-1 — morceau 1 de 10J — la maison rouge : il pose la premiere ligne, puis la piece entiere est ecrite

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **9 s** · **son ON** |
| **`@image1`** (start image) | **IMAGE E v2** (sh.1 |
| **`@video1`** | le clip précédent |
| **Éléments** | `@sam` · `@redhouseinterior` |

> ✂ **10J a été coupé en trois** : Seedance 2.0 plafonne à 15 s et le plan en fait 19. Ce morceau couvre la maison rouge : il pose la premiere ligne, puis la piece entiere est ecrite. Le montage garde les mêmes 9,5 s au total.

```
MATERIALS — @video1 is the shot immediately before this one (10I). @image1 IS THE FIRST FRAME OF THIS GENERATION. It is NOT the last frame of the attached video: this shot is taken from a different angle and its opening composition is the one @image1 gives. @sam is SAM. @redhouseinterior is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the attached video take ONLY the film stock, the grain structure, the way skin and fabric resolve, the focus behaviour and the highlight roll-off. DO NOT TAKE its light, its palette, its exposure, its composition or its framing, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From SAM and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — A man lays the first stroke of dark red on a bare wall and steps back to measure what it will cost.
CONTINUITY — ALIGN THE BOUNDARY BEFORE ANYTHING NEW HAPPENS: this clip opens on the INHERITED STATE line below exactly, everything in it already true and nothing replayed, and only then does the action begin. It must CONNECT NATURALLY, NOT IDENTICALLY.
INHERITED STATE, ALREADY TRUE AT FRAME ONE — SAM: centre of frame, STANDING, x=58%, filling 74% of frame height. Nothing in this list may be re-placed, re-lit or improved.
OPENING FRAME — Wide, camera at eye level 155 cm, 3 m back from the wall, static, locked off, the whole height of the room in frame. SAM at x=40%, filling 60% of frame height, three-quarter back; the bare wall beyond him; the bulb small and hard at the top of frame at x=64%.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-2.5s] He stands facing the bare wall, hands empty; HE LAYS THE FIRST LINE at x=34%, 26% of frame height, high on the wall: one horizontal run of dark red about forty centimetres long, the paint thick and dragging where the plaster is rough, ending in a thinning tail — ONE mark on an entirely bare wall, abstract, forming no letter, no character, no word, in no language. [2.5-5.1s] The brush comes off, his arm lowers, HE TAKES ONE STEP BACKWARDS and stops; NOT A CENTIMETRE OF BARE PLASTER IS LEFT: every wall from skirting to ceiling, the staircase risers, the underside of the handrail, the inside of the door, covered edge to edge in dense dark red looping strokes, layer over layer — a texture forming no characters, no letters, no words, in no language. [5.1-7.6s] The last white disappears under the red; ONE FULL BEAT OF DARK, only his breathing and the doorway's weak grey behind him.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 28 mm f/1.4 equivalent. CAMERA HOLD, and it is not optional: the camera stays Wide, camera at eye level 155 cm, 3 m back from the wall, static, locked off, the whole height of the room in frame.
LIGHT — TWO STATES, NOTHING IN BETWEEN.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The library service corridor, the grey window at its far end: a tired heavy man in a plain grey jacket, three-quarter back to camera, walking away down the parquet with a tight dark bundle under his left arm, shoulders down; the trolley beside him, nothing legible anywhere, nobody else in frame.
AVOID — the man's face, his profile, his eyes, him turning toward camera, a face above the coat's collar, the coat pausing or turning back, readable handwriting, legible letters on the walls, readable words anywhere, readable menus.
```

## 10J-2 — morceau 2 de 10J — la cuisine d'Anna puis le restaurant : il demande, on accepte

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **6 s** · **son ON** |
| **`@image1`** (start image) | **IMAGE E v2** (sh.1 |
| **`@video1`** | le clip précédent |
| **Éléments** | `@sam` · `@anna` · `@mei` · `@fatiha` · `@asha` · `@AnnaKitchen` · `@Restaurant` |

> ✂ **10J a été coupé en trois** : Seedance 2.0 plafonne à 15 s et le plan en fait 19. Ce morceau couvre la cuisine d'Anna puis le restaurant : il demande, on accepte. Le montage garde les mêmes 9,5 s au total.

```
MATERIALS — @video1 is the shot immediately before this one (10I). @image1 IS THE FIRST FRAME OF THIS GENERATION. It is NOT the last frame of the attached video: this shot is taken from a different angle and its opening composition is the one @image1 gives. @sam is SAM. @anna is ANNA. @mei is MEI. @fatiha is FATIHA. @asha is ASHA. @AnnaKitchen is ANNA KITCHEN, one of the places this shot happens in. @Restaurant is RESTAURANT, one of the places this shot happens in.
ROLES — From the attached video take ONLY the film stock, the grain structure, the way skin and fabric resolve, the focus behaviour and the highlight roll-off. DO NOT TAKE its light, its palette, its exposure, its composition or its framing, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From SAM, ANNA, MEI, FATIHA, ASHA and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — A man lays the first stroke of dark red on a bare wall and steps back to measure what it will cost.
CONTINUITY — ALIGN THE BOUNDARY BEFORE ANYTHING NEW HAPPENS: this clip opens on the INHERITED STATE line below exactly, everything in it already true and nothing replayed, and only then does the action begin. It must CONNECT NATURALLY, NOT IDENTICALLY.
INHERITED STATE, ALREADY TRUE AT FRAME ONE — SAM: centre of frame, STANDING, x=58%, filling 74% of frame height. Nothing in this list may be re-placed, re-lit or improved.
OPENING FRAME — Wide, camera at eye level 155 cm, 3 m back from the wall, static, locked off, the whole height of the room in frame. SAM at x=40%, filling 60% of frame height, three-quarter back; the bare wall beyond him; the bulb small and hard at the top of frame at x=64%.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-1.5s] BROAD DAYLIGHT, the stove unlit. SHE LISTENS TO THE END, then NODS — once slowly, and again smaller — her eyes steady on him, no surprise in her face, the face of someone being asked for a favour she was always going to do. [1.5-3.1s] Flat grey afternoon. [3.1-4.6s] Flat grey afternoon.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 28 mm f/1.4 equivalent. CAMERA HOLD, and it is not optional: the camera stays Wide, camera at eye level 155 cm, 3 m back from the wall, static, locked off, the whole height of the room in frame.
LIGHT — TWO STATES, NOTHING IN BETWEEN.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The library service corridor, the grey window at its far end: a tired heavy man in a plain grey jacket, three-quarter back to camera, walking away down the parquet with a tight dark bundle under his left arm, shoulders down; the trolley beside him, nothing legible anywhere, nobody else in frame.
AVOID — the man's face, his profile, his eyes, him turning toward camera, a face above the coat's collar, the coat pausing or turning back, readable handwriting, legible letters on the walls, readable words anywhere, readable menus.
```

## 10J-3 — morceau 3 de 10J — le couloir de la bibliotheque : le manteau quitte derriere le chariot

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **4 s** · **son ON** |
| **`@image1`** (start image) | **IMAGE E v2** (sh.1 |
| **`@video1`** | le clip précédent |
| **Éléments** | `@sam` · `@mender` · `@LibraryCorridor` |

> ✂ **10J a été coupé en trois** : Seedance 2.0 plafonne à 15 s et le plan en fait 19. Ce morceau couvre le couloir de la bibliotheque : le manteau quitte derriere le chariot. Le montage garde les mêmes 9,5 s au total.

```
MATERIALS — @video1 is the shot immediately before this one (10I). @image1 IS THE FIRST FRAME OF THIS GENERATION. It is NOT the last frame of the attached video: this shot is taken from a different angle and its opening composition is the one @image1 gives. @sam is SAM. @mender is THE MENDER. @LibraryCorridor is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the attached video take ONLY the film stock, the grain structure, the way skin and fabric resolve, the focus behaviour and the highlight roll-off. DO NOT TAKE its light, its palette, its exposure, its composition or its framing, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From SAM, THE MENDER and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — A man lays the first stroke of dark red on a bare wall and steps back to measure what it will cost.
CONTINUITY — ALIGN THE BOUNDARY BEFORE ANYTHING NEW HAPPENS: this clip opens on the INHERITED STATE line below exactly, everything in it already true and nothing replayed, and only then does the action begin. It must CONNECT NATURALLY, NOT IDENTICALLY.
INHERITED STATE, ALREADY TRUE AT FRAME ONE — SAM: centre of frame, STANDING, x=58%, filling 74% of frame height. Nothing in this list may be re-placed, re-lit or improved.
OPENING FRAME — Wide, camera at eye level 155 cm, 3 m back from the wall, static, locked off, the whole height of the room in frame. SAM at x=40%, filling 60% of frame height, three-quarter back; the bare wall beyond him; the bulb small and hard at the top of frame at x=64%.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-0.7s] THE COAT IS ALREADY IN FRAME AND ALREADY MOVING — no entrance, no start-up: a tall figure seen from DIRECTLY BEHIND at x=48%, filling 72% of frame height, turning the corner into the corridor, the long coat a dark umber-brown mass against the window's backlight with thousands of fine ivory lines rippling across its back, the deep hood up and the head completely lost inside it. [0.7-1.5s] HE DRAGS THE COAT UP OVER HIS HEAD in one movement — arms crossing, the hem coming up past the hips and shoulders, THE WHOLE GARMENT PASSING COMPLETELY ACROSS AND OVER HIS HEAD AND FACE and covering them entirely for the full duration of the movement, so not one frame shows both the coat and any part of his face. [1.5-2.2s] He ROLLS IT TIGHT under his left arm — a dull dark bundle, no line legible in it any more — and CROUCHES behind the trolley at x=62%, his back to camera, his head below its top shelf.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 28 mm f/1.4 equivalent. CAMERA HOLD, and it is not optional: the camera stays Wide, camera at eye level 155 cm, 3 m back from the wall, static, locked off, the whole height of the room in frame.
LIGHT — TWO STATES, NOTHING IN BETWEEN.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The library service corridor, the grey window at its far end: a tired heavy man in a plain grey jacket, three-quarter back to camera, walking away down the parquet with a tight dark bundle under his left arm, shoulders down; the trolley beside him, nothing legible anywhere, nobody else in frame.
AVOID — the man's face, his profile, his eyes, him turning toward camera, a face above the coat's collar, the coat pausing or turning back, readable handwriting, legible letters on the walls, readable words anywhere, readable menus.
```

## 10K — plan 10.10 « L'hôpital, enfin sonore » *(8 s · CADRE RÉFÉRENT ABSOLU · @MaeveIll + @SamBefore + @HospitalCorridor (LIEU-05) + @HospitalRoom (LIEU-04) · 50 mm f/2 · start frame : LIEU-05 IMAGE 3)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **8 s** · **son ON** |
| **`@image1`** (start image) | LIEU-05 IMAGE 3 |
| **`@video1`** | — aucune vidéo, tête de chaîne |
| **Éléments** | `@sambefore` · `@HospitalCorridor` · `@HospitalRoom` |

> 🔴 **Élément(s) inexistant(s) sur le compte : @MaeveIll.** Ils sont écrits en clair dans le prompt (le modèle les construira depuis la description), mais l'identité ne sera pas verrouillée d'un plan à l'autre tant qu'ils ne sont pas créés.

```
MATERIALS — @image1 IS THE FIRST FRAME OF THIS GENERATION. No previous clip is attached: this shot is the head of its chain and sets the light, the grain and the skin rendering the following shots match. @sambefore is SAM. @HospitalCorridor is HOSPITAL CORRIDOR, one of the places this shot happens in. @HospitalRoom is HOSPITAL ROOM, one of the places this shot happens in.
ROLES — From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From SAM and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — Through the glass of a hospital room door: a dying woman takes her husband's face in both her hands, will not let go of it, and asks him for something.
OPENING FRAME — Medium shot through the wired-glass door window, eye level from the corridor, static, locked off. The framing, camera height and off-centre crop never change by a single pixel.
ACTION in 4 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-1.9s] Her two hands rise slowly from the sheet — the cannula line following the left one — and settle on his cheeks, one each side, HER THUMBS ON HIS CHEEKBONES. [1.9-3.9s] Nobody moves. [3.9-5.8s] His shoulders go once — a single shake — and the light catches wet on his cheek. SHE GRIPS HARDER, the fingers pressing into his cheeks, her breath short, and speaks the second line. [5.8-7.7s] HIS MOUTH OPENS — the jaw comes down, the lips part on a word — AND NOTHING COMES OUT. He nods, slowly, inside her hands.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 50 mm f/2 equivalent. CAMERA HOLD, and it is not optional: the camera stays Medium shot through the wired-glass door window, eye level from the corridor, static, locked off.
LIGHT — Corridor side: cold green fluorescent tubes overhead behind the camera, even and shadowless, their reflections lying across the wired glass and sliding faintly.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — ⚠ ABSOLUTE REFERENCE FRAME — through the wired glass, her two hands holding his face with the thumbs on his cheekbones, her eyes on his, his cheek wet in the lamp's warmth, the crimson scarf muted on her shoulders, the green reflections lying across the pane.
AVOID — the man speaking, any male voice, a whisper from him, him wiping his face, his hands rising to hers, her hands leaving his face or coming down, more than two nods, him standing, him turning toward the door, a chair in frame.
```

## 10L — plan 10.11 **LE DERNIER PLAN DU FILM** « Il lève les yeux, et il sourit » *(8 s de rushes → 6 s au montage · @SamSDF + @Sam + @BusShelter · UNE SEULE PRISE, caméra fixe · 85 mm f/2 · pas de start frame)*

| | |
|---|---|
| **modèle** | **Seedance 2.0** · mode `std` · genre `drama` |
| **format** | 21:9 · **1080p** · bitrate **high** |
| **durée** | **8 s** · **son ON** |
| **`@image1`** (start image) | aucune — ce plan démarre sans image de départ |
| **`@video1`** | le clip précédent |
| **Éléments** | `@sam` · `@BusShelter` |

> 🔴 **Élément(s) inexistant(s) sur le compte : @SamSDF.** Ils sont écrits en clair dans le prompt (le modèle les construira depuis la description), mais l'identité ne sera pas verrouillée d'un plan à l'autre tant qu'ils ne sont pas créés.

```
MATERIALS — @video1 is the shot immediately before this one (10H). @image1 IS THE FIRST FRAME OF THIS GENERATION. It is NOT the last frame of the attached video: this shot is taken from a different angle and its opening composition is the one @image1 gives. @sam is SAM. @BusShelter is THE PLACE THIS SHOT HAPPENS IN.
ROLES — From the attached video take the light level and direction, the grain, the skin rendering and the way the camera behaves. TAKE NO FRAMING FROM IT beyond the first instant, and TAKE NOTHING OF ITS AUDIO: not its dialogue, not its voices, not one line spoken in it. Every line heard in it belongs to the previous shot and MUST NEVER BE HEARD AGAIN HERE. From the first-frame image take the opening composition and the exact position, pose and prop state of everything in it; take nothing else from it — no border, no backdrop, no empty-room staging, no reference layout. From SAM and the place reference take identity, face, build, wardrobe, architecture and materials exactly as the reference gives them; take none of their pose, framing, lighting or staging.
SHOT — A heavy homeless man alone on a bus shelter bench under sodium, his face already wiped in lighter streaks.
CONTINUITY — ALIGN THE BOUNDARY BEFORE ANYTHING NEW HAPPENS: this clip opens on the first-frame image exactly, everything in it already true and nothing replayed, and only then does the action begin. It must CONNECT NATURALLY, NOT IDENTICALLY.
OPENING FRAME — Close, straight on, camera 110 cm high, 1.1 m away, locked off, ONE TAKE WITH NO CUT AND NO REFRAMING. His head at x=48%, filling 64% of frame height; the clouded glass and the black empty road behind; the sodium raking down from above and slightly in front of him.
ACTION in 3 stages, timings are budgets not edit points, each stage ends on the state the next one starts from — [0.0-1.9s] Start positions, held until described otherwise: SEATED, bare-headed, hood back off his head, the beanie shut in his left fist on his knee, his head lowered toward his knees, THE FACE ALREADY WIPED — the grime lifted in bands so the skin reads lighter and blotched across the forehead and both cheekbones, the hair soaked flat with sweat. [1.9-3.9s] A LONG BEAT, and nothing else happens: he holds the sky, breathing, two slow blinks, the jaw loose. [3.9-5.8s] THE CORNERS OF THE MOUTH GO UP AND DO NOT COME DOWN AGAIN — a small, uneven, involuntary smile that stays.
PERFORMANCE — Nobody performs an emotion here. The feeling is legible only through the exact physical events written in the ACTION stages above, and through nothing else: no added expression, no widened eyes, no trembling lip, no theatrical breath.
CAMERA — Anamorphic 85 mm f/2 equivalent. CAMERA HOLD, and it is not optional: the camera stays Close, straight on, camera 110 cm high, 1.1 m away, locked off, ONE TAKE WITH NO CUT AND NO REFRAMING.
LIGHT — one source only: a single sodium street lamp on a column above the shelter, the column out of the top of frame, mounted SLIGHTLY FORWARD OF THE BENCH LINE toward the road, that is, slightly on the camera's side, so the hard orange cone falls from above and a little toward the lens and never from behind — this is the whole point of the lighting: the brow does not shadow the eye sockets.
SOUND — NOBODY SPEAKS IN THIS ENTIRE GENERATION: not one line, not one word, not one murmur, and NO LINE IS CARRIED OVER FROM THE ATTACHED VIDEO. SFX only.
ENDING FRAME, the state this shot hands over — The bearded man alone on the bench under the sodium cone, bare-headed, the beanie shut in his left fist on his knee, his face wiped in lighter streaks — his head lifted well above level and his pale grey-blue eyes wide open on the night sky out of frame, the corners of his mouth up and staying up, and one single line of water down the wiped cheek, not wiped away.
AVOID — a false beard, a glued-on beard, a prosthetic beard, a beard being pulled off, a mask, a wig, a disguise being removed, anything peeling from the face, a glue line, the beanie thrown away or left on the bench.
```
