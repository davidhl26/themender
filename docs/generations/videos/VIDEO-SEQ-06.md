...
> **@Sam** *(pack §1 — 6C seulement, en silhouette jamais éclairée)* · lieux **@NoraBedroom**
> (docs/generations/lieux/LIEU-08 — le bureau ET le palier), **@BackGallery** (LIEU-09).
> **Start frames** : **LIEU-08 IMAGE 3** (ANGLE B, le bureau, laptop OUVERT) → 6A (shot 1 ; les shots 2 et 3
> sont décrits au prompt) et → 6A-bis · **LIEU-09 IMAGE 2** (ANGLE A, le long de la galerie) → 6B (shot 1 ;
> l'IMAGE 3 est l'axe du shot 2) · **LIEU-09 IMAGE 3** (ANGLE B, au rail) → 6B-bis ·
> **LIEU-08 IMAGE 4** (ANGLE C, le palier, porte entrouverte 10 cm) → 6C.
> **Ordre au montage : 6A → 6B → 6C.** Le raccord 6A→6B est un cut nuit/nuit : son regard qui se lève vers la
> fenêtre noire amène la galerie. 6B→6C : elle rentre par la porte de la galerie, on la retrouve derrière sa
> porte de chambre — l'ellipse de l'escalier est dans le cut.
> ⚠ Générer DANS LE PROJET FESTIVAL. Un bloc = une génération, collée telle quelle, sans en retirer une ligne.
>
> ⚠ **CE QUE LA REFONTE DU 31/08 CHANGE ICI — à lire avant de générer.**
> · **La séquence tombe de 10+9+5 à 7+9+5 = 21 s.** Seul le forum est touché.
> · **Le forum ne porte plus que DEUX brèves au lieu de trois, et LA LOI EST RETIRÉE** (« il demande quelque
>   chose d'abord ») : les trois femmes viennent de la dire en 5.2, deux plans plus tôt. **Il ne reste au forum
>   qu'un seul boulot : donner le rituel.** Trois pavés de texte à lire en 10 s, c'était trop.
>   **Conséquence directe sur le [POST] : ne composer AUCUN bloc de loi, nulle part, dans aucun état de l'écran.**
> · **6.3 est GARDÉ intégralement.** C'est lui qui prouve que Sam était là, réveillé, à minuit — le spectateur
>   ne le relira qu'à la fin. Ne pas le raccourcir, ne pas l'« expliquer ».
> · Aucun autre plan de forum n'existe dans le film après 6.1 : l'ancien 7.3 (l'écran sans réponse) est supprimé.
>
> ⚠ **LES QUATRE ÉCARTS DÉCLARÉS AUX START FRAMES** *(à connaître avant de lancer — un écart non déclaré est un raccord perdu)*
> · **6A / LIEU-08 IMAGE 3** : l'image de référence est **vide**, la chaise de bureau inoccupée. La vidéo assied
>   @Nora dans cette chaise exacte ; les shots 2 et 3 sont des axes que le lieu n'a pas et qui sont décrits au prompt.
> · **6A / l'écran** : le lieu décrit l'écran comme un rectangle bleu **flou**. Ici, le shot 2 le veut **NET et
>   totalement vierge de caractères** — c'est la plaque sur laquelle le POST vient se caler. **Écart assumé et
>   indispensable** : du texte composé sur un écran flou ne tient pas, et c'est ce plan qui porte le rituel.
> · **6B / LIEU-09 IMAGE 2** : la galerie de référence est vide et le reste 1,2 s ; @Nora y entre ensuite. Le
>   shot 2 change de valeur pour l'axe de l'IMAGE 3, plus serré — **le rectangle chaud du dessous sort alors du
>   cadre de lui-même**, ce qui est exactement l'effet recherché.
> · **6C / LIEU-08 IMAGE 4** : l'image de référence est vide et sa porte immobile. La vidéo y ajoute deux corps
>   — dont un que la lumière ne touche jamais — et **la porte se referme en fin de plan**, la lame de lumière
>   mourant avec elle. **Aucune source n'est ajoutée** : la lame reste l'unique source du lieu.
>
> ⚠ **VERROUS DE LA SÉQUENCE**
> · **RÈGLE G — le seul [POST] du film sur cette séquence.** Aucun texte lisible n'est généré nulle part.
>   L'écran est généré en **lueur bleue + blocs pâles illisibles, jamais un mot, jamais un caractère** ; les deux
>   brèves, la carte aux huit épingles et le rituel sont **composés en post**, curseur cyan compris.
> · **RÈGLE A** — aucun des textes composés ne contient *story*, *stories*, *tale*, *legend*, *myth*, ni aucun
>   synonyme. Le mot est mort avec Maeve ; il ne revient que dans sa bouche, à la fin du film.
> · **RÈGLE B** — **aucun rouge dans la séquence, nulle part.** Le curseur est cyan (post). Aucune diode rouge de
>   chargeur, aucun feu arrière, aucune enseigne, aucun voyant.
> · **RÈGLE D** — **le Mender n'existe pas dans cette séquence.** Aucun manteau, aucun dos, aucune écriture, dans
>   aucun des cinq blocs. En 6C, le visage de Sam **n'est jamais éclairé** : l'ambiguïté est le sujet.
> · **RÈGLE ZÉRO (6C)** — elle répond et elle referme. **Elle ne pose aucune question, ne prononce aucun nom, ne
>   cherche rien sur le visage de son père, ne le regarde pas partir.** Pas un frame de doute.
> · **6B — elle est SEULE.** Caméra strictement fixe, aucun mouvement d'aucune sorte, **aucune fenêtre allumée
>   jamais cadrée directement, aucune silhouette dans aucune fenêtre**, et le rectangle chaud du dessous reste au
>   bord bas du cadre large sans être jamais souligné. Rien ne répond au cri : ni voix, ni écho, ni lumière qui change.
> · **AUCUN SOUS-TITRE** : chaque NEGATIVE PROMPT porte *subtitles, captions, burned-in text, karaoke lyrics*.
>
> ⚠ **LE [POST] DE LA SÉQUENCE — textes à composer, et rien d'autre** *(sur la plaque du shot 2 de 6A)*
> La plaque contient **UN SEUL scroll**, et ce scroll est le passage d'un état à l'autre de la page composée :
> · **ÉTAT A** *(avant le scroll)* — les deux brèves, une ligne chacune, et sous elles la carte aux huit épingles :
>   *« He asked my father for a chair. My father hadn't stood up in a year. »* ·
>   *« He asked me for salt. »* · la carte, huit épingles, huit vignettes du même dos.
> · **ÉTAT B** *(après le scroll, en gras, plein cadre, au moins 2 s à l'écran)* —
>   *« If you carry a why too heavy: at midnight, cry it out to the night, out loud. »*
> · Curseur **cyan** clignotant, et rien d'autre. **Aucun bloc de loi, aucun troisième témoignage, aucun compteur,
>   aucun nom d'utilisateur, aucune date.** Les huit vignettes du dos sont des dessins au trait composés en post —
>   **jamais un manteau généré à l'image** (règle D).
...
```
Storyboard sheet with 3 sequential panels in a clean grid layout, thin white borders between panels, each panel labeled only with its shot code (6A, 6B, 6C) in small letters under the panel. Cinematic live-action film previsualization: realistic human characters, consistent face, wardrobe and locations across all panels, a desaturated grey-blue world, gently underexposed, sparse motivated sources, deep blacks — cold laptop blue in a dark bedroom, then sodium orange over a night gallery, then a near-black landing. Panel 1 ("6A"): a fifteen-year-old girl's face front-on filling the frame in a dark bedroom, lit cold blue-white from just below, the laptop itself out of frame under the lower edge, a thin sodium-orange rim in her hair from a window at screen-right, her eyes lifted just past the unseen screen toward the dark window, lips closed, everything behind her blue-black. Panel 2 ("6B"): the rear wooden gallery at night seen along its length, the girl from behind at the painted rail at mid-frame in a dark navy parka, both hands off the rail and her body already turning back toward the ajar door at the right frame edge, the empty sagging washing line above, dark rooftops and far harbour cranes beyond, the warm rectangle of window light lying low and unremarked at the bottom frame edge, nothing else in the world lit. Panel 3 ("6C"): the near-black upstairs landing, the bedroom door at screen-right now shut and the blade of light gone, the father a grey unlit silhouette at frame-left facing the closed door, arms at his sides, barely separable from the dark, no light of any kind on his face. No logos, no brand names, no other text anywhere beyond the shot code labels. No readable characters on any screen in panel 1. No red anywhere in any panel. No long written coat and no coated figure anywhere in any panel. Hyper-realistic cinematic film stills, true-to-life colour, film grain — NOT motion graphics, NOT an illustration, NOT concept art, no CGI look. Real photographic texture in every panel: natural depth of field, halation around practical light sources, faint noise in the shadows, film-like highlight roll-off, imperfect human framing; no oversharpening, no waxy skin, no teal-and-orange colour grade, no AI artifacts.
```

---

## VIDÉO 6A — plan 6.1 « Le forum » *(12 s de rushes → 7 s au montage · Elements : @Nora + @NoraBedroom · start frame : LIEU-08 IMAGE 3 — écarts déclarés en tête de document)*

```
Style prompt: A fifteen-year-old girl's small unmade bedroom, late at night, desaturated grey-blue. The open laptop screen is the only key light — cold blue-white on her face, shifting faintly as unseen pages move — with the sodium orange of a street lamp through the window as a thin rim on her hair, and deep blue-black shadow everywhere else, gently underexposed, blacks truly black, the corners of the room gone entirely. The screen carries no writing of any kind: only a soft blue field and a few pale grey blocks with no characters in them, not one letter, not one digit, in any shot, at any moment. Camera locked off in every shot, no handheld shake. Real-time pacing. Hyper-realistic live-action footage, indistinguishable from film shot on a real set with real actors: true skin texture with visible pores and fine hairs, real fabric weave, seams and wrinkles that crease with movement, natural motion blur, constant micro-life in every held position (breathing, blinking, weight shifts of a few millimetres), only motivated practical light, subtle film grain, true-to-life colour. NOT CGI, NOT animation, no uncanny smoothness, no plastic skin, no beauty retouching, no video-game look.

SCENE Alone at night in a dark bedroom, a girl reads a page on her laptop, scrolls once, stops on something, and lifts her eyes off the screen toward the black window. Three shots, two hard cuts. Nothing on the screen is ever a word: the page exists as light and blocks only. Every second is choreographed below; nothing beyond it may be invented.
...
POSTURE LOCK @Nora is SEATED at the desk for the entire generation and never stands, never leaves the chair, never turns away from the desk. Shoulders rounded inward, hood down, hair pushed behind the ears. Her hands: left forearm flat on the desk edge and staying there; right hand resting at the trackpad, the index finger moving only for the single scroll written below. She wears the oversized faded charcoal-grey hoodie — no parka in this generation. She never types, never speaks, never touches her face.
...
```
Style prompt: The open laptop screen of a fifteen-year-old girl's dark bedroom at night, seen alone, nobody in the room. Desaturated grey-blue darkness all around it; the screen is the only light in frame — a plain, even, cold blue-white panel that carries no writing of any kind: only a soft blue field and a few pale grey blocks with no characters in them, not one letter, not one digit, at any moment. One single shot, no cut. Camera locked off, absolutely no drift, no handheld shake. Real-time pacing. Hyper-realistic live-action footage, indistinguishable from film shot on a real set with real actors: true skin texture with visible pores and fine hairs, real fabric weave, seams and wrinkles that crease with movement, natural motion blur, constant micro-life in every held position (breathing, blinking, weight shifts of a few millimetres), only motivated practical light, subtle film grain, true-to-life colour. NOT CGI, NOT animation, no uncanny smoothness, no plastic skin, no beauty retouching, no video-game look.

SCENE A laptop screen alone in a dark empty bedroom, holding a page that is only light and blocks. It sits still, scrolls once, and sits still again. One single shot, no cut, closer than the main version and without anyone in frame, so the surface is clean enough to be composed on. Nobody appears at any point. Every second is choreographed below; nothing beyond it may be invented.

PERFORMANCE PACING Unhurried, naturalistic, lived-in performance throughout. Real-time pacing: every action takes the time it takes in real life, nothing is compressed or hurried. A character always finishes their action before they speak. Before every reply there is a beat of silence — the listener breathes, thinks, then answers. Dialogue at relaxed conversational speed with natural pauses inside the lines. Held looks are truly held. Documentary naturalism, warm and imperfect, never theatrical.

NATURAL MICRO-MOVEMENT Nobody is ever frozen, not for a single frame. Between and during the written actions, every character carries constant, very slow, very human micro-movement: breath visibly lifting the chest and shoulders, soft weight shifts of a few millimetres, small head drifts of a degree or two, fingers settling and resettling, eyes travelling a few millimetres and returning, blinks at natural intervals. This micro-life never adds gestures, steps, props or expressions — it is simply how a living body holds a position. Always slow and organic, never fidgety, never twitchy; no statue stillness, no freeze-frame, no mannequin pose. In this generation no person is present, so this rule applies to nothing: the frame is a still object and stays a still object, alive only through the panel's own faint light and the grain of the image.
...
POSTURE LOCK Nobody is in this generation. No person, no body, no hand, no shoulder, no reflected figure, in any frame.
...
```
Style prompt: The rear wooden gallery of a working-class triple-decker at midnight, desaturated grey-blue. Sodium street lamps from below and behind push a dull orange up-light that catches the underside of the rail, the curled paint flakes and the proud rusted nailheads, and lights her breath into brief clouds; the night sky is a cold deep blue, nearly black at the zenith, and lies as a weak top fill on her shoulders; everything outside these two sources falls to near-black, gently underexposed, blacks truly black. The warm rectangle of window light from the flat below lies low across the boards near the bottom frame edge, one texture among others, never favoured, never approached, never framed on its own. Camera locked off in both shots, no movement of any kind, no handheld shake. Real-time pacing, long true silences. Hyper-realistic live-action footage, indistinguishable from film shot on a real set with real actors: true skin texture with visible pores and fine hairs, real fabric weave, seams and wrinkles that crease with movement, natural motion blur, constant micro-life in every held position (breathing, blinking, weight shifts of a few millimetres), only motivated practical light, subtle film grain, true-to-life colour. NOT CGI, NOT animation, no uncanny smoothness, no plastic skin, no beauty retouching, no video-game look.

SCENE At midnight a girl comes out onto the rear gallery of her building, takes hold of the rail, hesitates, and throws one question out loud at the rooftops. Nothing answers. A dog barks twice, very far away. She lets go and goes back in. Two shots, one hard cut. She is the only living being on screen and nothing in the world responds to her. Every second is choreographed below; nothing beyond it may be invented.
...
POSTURE LOCK @Nora is STANDING for the entire generation — she never sits, never crouches, never leans over the rail, never climbs anything. She wears the dark navy nylon parka with the broken zip pull over the charcoal hoodie, hood down. Her hands are empty throughout: at her sides from [0.0s] to [2.6s], BOTH ON THE RAIL from [2.6s] to [10.4s], at her sides again after. She holds no object at any point.
...
```
Style prompt: A close section of the rear wooden gallery of a working-class triple-decker at midnight, desaturated grey-blue, framed so tightly that no window, no door and no walkway can enter the shot. Sodium street lamps from below and behind rake their dull orange upward, catching the underside of the rail, the curled paint flakes and the proud rusted nailheads, edging her jaw and shoulder from behind and lighting her breath into brief clouds; the cold deep blue of the night sky lies as a weak top fill; everything else falls to near-black, gently underexposed, blacks truly black. One single shot, no cut. Camera locked off, no movement of any kind, no handheld shake. Real-time pacing, long true silences. Hyper-realistic live-action footage, indistinguishable from film shot on a real set with real actors: true skin texture with visible pores and fine hairs, real fabric weave, seams and wrinkles that crease with movement, natural motion blur, constant micro-life in every held position (breathing, blinking, weight shifts of a few millimetres), only motivated practical light, subtle film grain, true-to-life colour. NOT CGI, NOT animation, no uncanny smoothness, no plastic skin, no beauty retouching, no video-game look.

SCENE At midnight, at the rail of a rear gallery, a girl hesitates and throws one question out loud at the rooftops. Nothing answers. A dog barks twice, very far away. One single shot, no cut, tighter than the main version so that nothing in the world can enter the frame to answer her. She is the only living being on screen. Every second is choreographed below; nothing beyond it may be invented.
...
POSTURE LOCK @Nora is STANDING for the entire generation — she never sits, never crouches, never leans over the rail, never steps away, never turns her back to camera. She wears the dark navy nylon parka with the broken zip pull over the charcoal hoodie, hood down. Both hands are on the rail from [0.0s] to the end, knuckles pale with grip; she holds no object at any point.

FRAME MAP & CHOREOGRAPHY — SECOND BY SECOND

SHOT 1 [0.0-6.0s] — Medium close along the rail, three-quarter left profile, eye level, camera about 160 cm high and 1.2 m from her, static, locked off, one stop tighter than the main version so the frame is filled by her, the rail and the night alone.
...
AUDIO Night wind, the low tide-hum of the port beneath everything, a loose halyard clinking far off against a mast, her breathing audible and quick; then the line — one voice alone in the night, dry and unamplified, no reverb, no echo, no space added to it — and after it a long true silence with nothing in it but the wind, broken at [4.4s] by a dog barking twice, very far away. No reply of any kind. No door, no window, no voice. No music.
...
```
Style prompt: A near-black upstairs landing at night in a working-class family home, at the top of the stairs. The only light is the thin blade of cold blue and sodium light escaping through the girl's bedroom door, ajar ten centimetres, lying across the bare floorboards toward camera. The man exists only as an unlit grey silhouette against deeper black — no light of any kind ever touches his face, at any point, and his expression never resolves. Deep crushed blacks, desaturated, gently underexposed, sparse motivated sources. Camera locked off, no handheld shake. Real-time pacing, long true silences. Hyper-realistic live-action footage, indistinguishable from film shot on a real set with real actors: true skin texture with visible pores and fine hairs, real fabric weave, seams and wrinkles that crease with movement, natural motion blur, constant micro-life in every held position (breathing, blinking, weight shifts of a few millimetres), only motivated practical light, subtle film grain, true-to-life colour. NOT CGI, NOT animation, no uncanny smoothness, no plastic skin, no beauty retouching, no video-game look.

SCENE A father stands in the dark landing outside his daughter's door, just arrived at the top of the stairs, and asks one quiet question. She answers through the gap without opening it. The door closes. He stays where he is. One single static shot, no cut. Whether he heard anything tonight is never answered — his face is never lit, his expression never readable, and she never asks him anything. Every second is choreographed below; nothing beyond it may be invented.
...
POSTURE LOCK @Sam is STANDING for the entire generation, hands empty at his sides, and never moves from where he stands — he never steps forward, never steps back, never turns away, never reaches for the door, never touches anything. He is rendered ONLY as an unlit grey silhouette: outline and posture, never features, never skin, never eyes. @Nora is STANDING behind the door for the entire generation, visible only as a vertical sliver in the ten-centimetre gap: one eye, part of a cheek, strands of dark hair, and the dark navy edge of her parka collar. She never opens the door wider, never steps into the gap, never leans out.
